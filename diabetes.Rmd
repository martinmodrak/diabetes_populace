...
```{r}
cd4_calibration_dfs_L2 <- calibration_df_all(cd4_all_prop_with_preliminary, "L2", calibration_cache_dir)
```
```{r,fig.width=8,fig.height=8}
...
```{r}
cd8_preliminary_comparison <- cd8_all_prop_with_preliminary %>% 
  mutate(isPreliminary = if_else(isPreliminary, "preliminary", "final")) %>%
  pivot_wider(names_from = isPreliminary, values_from = c("Sample_ID", "Experiment_ID", "n", "sum_parent", "sum_all", "prop_parent", "prop_all")) %>%
  filter(!is.na(Sample_ID_preliminary))

stopifnot(!any(is.na(cd8_preliminary_comparison$Sample_ID_final)))
#stopifnot(length(unique(cd8_preliminary_comparison$Patient_ID)) == 9)
```
...
```{r}
cd8_preliminary_comparison_l2 <- cd8_preliminary_comparison %>% 
  filter(Level == "L2") %>%
  group_by(Patient_ID, Condition) %>%
  mutate(fold_change = ((n_final + 1) / sum_all_final) /   ((n_preliminary + 1) / sum_all_preliminary),
         perturbation_change = fold_change / sum(fold_change)
...
```{r}
cd8_preliminary_comparison_l3 <- cd8_preliminary_comparison %>% 
  filter(Level == "L3") %>%
  group_by(Patient_ID, Condition) %>%
  mutate(fold_change = ((n_final + 1) / sum_all_final) /   ((n_preliminary + 1) / sum_all_preliminary))

cd8_preliminary_comparison_l3 %>%
  ggplot(aes(x = L3, y = fold_change, color = Condition)) + 
  geom_hline(yintercept = 1, color = "orangered") +
  geom_point(position = position_jitter(width = 0.1), alpha = 0.8) +
  scale_y_log10()  + coord_flip()
...
```{r}
cd8_calibration_dfs_L2 <- calibration_df_all(cd8_all_prop_with_preliminary, "L2", calibration_cache_dir)
```
```{r,fig.width=8,fig.height=8}
